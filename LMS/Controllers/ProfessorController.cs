    ...
    public class ProfessorController : Controller
    {
        ...
        public IActionResult GetStudentsInClass(string subject, int num, string season, int year)
        {
            var result = (from s in db.Students
                join e in db.Enrollments on s.UId equals e.StudentuId
                join cl in db.Classes on e.ClassId equals cl.ClassId
                join c in db.Courses on cl.CourseId equals c.CourseId
                where c.DeptSubject == subject && c.CourseNum == num && cl.SemesterSeason == season &&
        ...
        public IActionResult GetAssignmentsInCategory(string subject, int num, string season, int year, string? category)
        {
            var result = (from c in db.Courses
                ...
                where c.DeptSubject == subject && c.CourseNum == num && cl.SemesterSeason == season &&
                      cl.SemesterYear == year && (category == null || ac.Name == category)
                select new
                {
                    aname = a.Name,
                    cname = ac.Name,
        ...
        public IActionResult GetAssignmentCategories(string subject, int num, string season, int year)
        {
            var result = (from c in db.Courses
                join cl in db.Classes on c.CourseId equals cl.CourseId
                join ac in db.AssignmentCategories on cl.ClassId equals ac.ClassId
                where c.DeptSubject == subject && c.CourseNum == num && cl.SemesterSeason == season && cl.SemesterYear == year
                select new
                {
                    name = ac.Name,
        ...
        public IActionResult CreateAssignmentCategory(string subject, int num, string season, int year, string category, int catweight)
        {
            ...
            var existing = db.AssignmentCategories.FirstOrDefault(ac => ac.ClassId == classId && ac.Name == category);
        ...
        {
            var categoryId = (from c in db.Courses
                join cl in db.Classes on c.CourseId equals cl.CourseId
                join ac in db.AssignmentCategories on cl.ClassId equals ac.ClassId
                where c.DeptSubject == subject && c.CourseNum == num && cl.SemesterSeason == season &&
                      cl.SemesterYear == year && ac.Name == category
        ...
        public IActionResult GetSubmissionsToAssignment(string subject, int num, string season, int year, string category, string asgname)
        {
            var result = (from c in db.Courses
                ...
                join ac in db.AssignmentCategories on cl.ClassId equals ac.ClassId
                join a in db.Assignments on ac.CategoryId equals a.CategoryId
                join su in db.Submissions on a.AssignmentId equals su.AssignmentId
                join s in db.Students on su.StudentuId equals s.UId
                where c.DeptSubject == subject && c.CourseNum == num && cl.SemesterSeason == season &&
                      cl.SemesterYear == year && ac.Name == category && a.Name == asgname
                select new
                {
                    fname = s.FName,
                    lname = s.LName,
                    uid = s.UId,
                    time = su.SubmissionTime,
                    score = su.Score
                });
        ...
        {
            var submissisonToGrade = (from c in db.Courses
                    join cl in db.Classes on c.CourseId equals cl.CourseId
                    join ac in db.AssignmentCategories on cl.ClassId equals ac.ClassId
                    join a in db.Assignments on ac.CategoryId equals a.CategoryId
                    join su in db.Submissions on a.AssignmentId equals su.AssignmentId
                    where c.DeptSubject == subject && c.CourseNum == num && cl.SemesterSeason == season && cl.SemesterYear == year && ac.Name == category && a.Name == asgname && su.StudentuId == uid
                        select su).FirstOrDefault();
            
            if (submissisonToGrade == null)
                return Json(new { success = false });
            
            submissisonToGrade.Score = (uint)score;
            db.SaveChanges();
